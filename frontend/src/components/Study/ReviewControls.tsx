...
export const ReviewControls: React.FC<ReviewControlsProps> = ({ isFlipped, onShow, onSubmit, intervals, isPractice }) => {
    ...
    if (isPractice) {
        return (
            <div className="w-full max-w-[600px] mt-4 md:mt-6 grid grid-cols-2 gap-3">
                <button
                    onClick={() => onSubmit(0)}
    ...
    return (
        <div className="w-full max-w-[600px] mt-4 md:mt-6 grid grid-cols-4 gap-2 md:gap-3">
            {ratings.map((rating) => (
                <button
                    key={rating.value}
                    onClick={() => onSubmit(rating.value)}
                    className={`${rating.color} text-white font-black py-2.5 md:py-5 rounded-xl active:scale-[0.98] flex flex-col items-center justify-center shadow-lg group`}
                >
                    ...
                    {intervals && intervals[rating.value] && (
                        <span className="text-[8px] md:text-[10px] opacity-70 font-bold px-1 md:px-2 py-0.5 bg-black/10 rounded-full">
