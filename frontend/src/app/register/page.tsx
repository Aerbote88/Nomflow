    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setError(data.detail || t('errorTaken'));
            }
        } catch (err) {
            logger.error(err);
            ...
            <GlassCard className="w-full max-w-md p-8 md:p-10 shadow-2xl relative overflow-hidden">
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                        ...
                        />
                        <p className="text-[10px] text-text-secondary/40 mt-1.5 px-1">{t('minChars')}</p>
                    </div>
                    ...
                    >
                        <span className="relative z-10">{loading ? t('creatingAccount') : t('createAccount')}</span>
                        {loading && (
                    ...
                    <p className="text-center text-[10px] font-black text-text-secondary/60 uppercase tracking-widest">
                        {t('haveAccount')} <Link href="/login" className="text-accent-primary hover:text-accent-primary/80 transition-colors underline decoration-accent-primary/30">{t('signIn')}</Link>
                    </p>
                </form>
            </GlassCard>
