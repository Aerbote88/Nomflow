    ...
    if (error || !stats) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
                <p className="text-text-secondary mb-6">{error || tc('errorGeneric')}</p>
            ...
            <div className="w-full max-w-4xl space-y-3 md:space-y-6">
                        ...
                        >
                            <div className="flex items-center justify-between w-full gap-2 md:gap-3 md:w-auto md:justify-start">
                                <div className="w-10 h-10 rounded-lg bg-accent-primary/10 flex flex-col items-center justify-center text-accent-primary border border-accent-gold/20 flex-shrink-0">
                                    <span className="text-[7px] font-black uppercase leading-none mb-0.5 opacity-70">{t('lvl')}</span>
                                ...
                                <div className="flex flex-col items-start translate-y-[1px] flex-1 md:flex-none">
                                    <span className="hidden md:block text-[9px] text-accent-primary uppercase font-black tracking-[0.2em] leading-none mb-2 opacity-80">{t('quickStats')}</span>
                                    <div className="flex items-center justify-around w-full md:justify-start md:gap-4">
                                        <div className="flex flex-col items-center md:items-start">
                                            <span className="text-[7px] text-text-secondary uppercase font-black tracking-widest leading-none mb-1 opacity-60">{t('studying')}</span>
                                            <span className="text-xs font-bold text-text-primary capitalize truncate max-w-[80px]">{stats.active_source_name || t('idle')}</span>
                                        </div>
                                        <div className="flex flex-col items-center md:items-start md:border-l md:border-white/10 md:pl-3">
                                            <span className="text-[7px] text-text-secondary uppercase font-black tracking-widest leading-none mb-1 opacity-60">{t('streak')}</span>
                                            <span className="text-xs font-bold text-orange-500">🔥 {stats.streak}</span>
                                        </div>
                                        <div className="flex flex-col items-center md:items-start md:border-l md:border-white/10 md:pl-3">
                                            <span className="text-[7px] text-text-secondary uppercase font-black tracking-widest leading-none mb-1 opacity-60">{t('due')}</span>
                        ...
                        <GlassCard className="col-span-2 md:flex-[2] md:min-w-[320px] !p-4 md:!p-8 border-accent-primary items-stretch">
                            <div className="flex justify-between items-end mb-4">
                                <div className="flex flex-col">
                                    <span className="text-xs md:text-sm text-text-secondary uppercase tracking-widest font-bold">{t('level')}</span>
                                    <span className="text-5xl md:text-6xl font-black leading-none text-text-primary">
                                        {stats.level}
                                    </span>
                                </div>
                                <div className="flex flex-col items-end">
                                    <span className="text-xs text-text-secondary uppercase font-bold tracking-wider">{t('totalXp')}</span>
                            ...
                            <div className="flex justify-between mt-2">
                                <span className="text-[10px] md:text-xs text-text-secondary font-bold uppercase tracking-tight">{t('level')} {stats.level}</span>
                                <span className="text-[10px] md:text-xs text-text-secondary font-bold uppercase tracking-tight">{t('level')} {stats.level + 1}</span>
                            </div>
                        </GlassCard>

                        <StatCard label={t('streak')} value={stats.streak} icon="🔥" colorClass="text-orange-500" />
                        <StatCard label={t('due')} value={stats.due_count} colorClass="text-pink-500" />
                        <StatCard label={t('learning')} value={stats.learning_count} colorClass="text-amber-500" />
                        <StatCard label={t('learned')} value={stats.learned_count} colorClass="text-emerald-500" />

                        {/* Active Source Card */}
                        <GlassCard className="col-span-2 md:flex-1 md:min-w-[240px] flex flex-col items-center justify-center !p-4 md:!p-6">
                            <span className="text-xs md:text-sm text-text-secondary uppercase tracking-widest font-bold mb-2">{t('studying')}</span>
                            <div className="text-xl md:text-3xl font-bold text-text-primary text-center font-serif leading-tight">
                                {stats.active_source_name || t('none')}
                            </div>
                ...
                <div className="grid grid-cols-2 gap-2 md:gap-4 w-full max-w-xl mx-auto mt-6 md:mt-12">
                    ...
                    >
                        ...
                        <span className="uppercase tracking-[0.2em] text-[10px] font-black">{t('library')}</span>
                    </Button>
                    <Button
                        variant="secondary"
                    ...
                    >
                        ...
                        <span className="uppercase tracking-[0.2em] text-[10px] font-black">{t('myFlashcards')}</span>
                    </Button>
                    <Button
                        variant="secondary"
                        className="flex flex-col py-3 md:py-8 group hover:border-accent-primary/50 hover:bg-accent-primary/5 transition-all duration-300"
                    ...
                    >
                        ...
                        <span className="uppercase tracking-[0.2em] text-[10px] font-black">{t('leaderboard')}</span>
                    </Button>
                    <Button
                        variant="secondary"
                        className="flex flex-col py-3 md:py-8 group hover:border-accent-primary/50 hover:bg-accent-primary/5 transition-all duration-300"
                    ...
                    >
                        ...
                        <span className="uppercase tracking-[0.2em] text-[10px] font-black">{t('settings')}</span>
                    </Button>
                </div>
            </div>
